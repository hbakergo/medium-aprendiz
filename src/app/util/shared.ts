...
export class Shared {
  ...
  public static initializeWebStorage(): void{
    ...
    //Local Storage de Themes
    if (localStorage.getItem(Constants.THEMES_KEY) === null){
      localStorage.setItem(Constants.THEMES_KEY, JSON.stringify([]));
    } else return;

    //Local Storage de Messages
    if (localStorage.getItem(Constants.MESSAGES_KEY) === null){
      localStorage.setItem(Constants.MESSAGES_KEY, JSON.stringify([]));
    } else return;
